module.exports = {

  categories : [
    "especialidades",
    "tecnologia",
    "equipo",
    "contacto",
  ],

  carouselData : [
    {
      title:"CONSULTORIO ESPECIALIZADO \nDE ALTA TECNOLOGIA",
      description: "Estamos especilizados en todas las áreas, lo que nos permite ofrecerte una atención multidisciplinaria en el mismo lugar"
    },
    {
      title:"FAMILIA DE PROFESIONALES",
      description: "Somos una familia de profesionales dedicados al cuidado y la prevención de tu salud bucal.",
    },
    {
      title: "TECNOLOGIA DIGITAL",
      description: "Estamos en constante capacitación y adquisición de nuevas tecnologías, para brindarte un servicio de calidad.",
    }
  ],

  // treatmentsList = [
  //   "caries" = [
  //     "text" = "lalal",
  //     "pic" = "lolo",
  //   ],
  //   "limpieza ultrasónica" = [
  //     "text" = "lalal",
  //     "pic" = "lolo",
  //   ],
  //   "exodoncias" = [
  //     "text" = "lalal",
  //     "pic" = "lolo",
  //   ],
  //   "blanqueamientos" = [
  //     "text" = "La técnica de blanqueamiento combinado realizada en nuestro consultorio permite el clareamiento dental de hasta 5 tonos, con la menor sensibilidad. Es un tratamiento con el que se obtienen resultados de manera rápida y se consiguen unos cambios muy notables en la sonrisa.",
  //     "pic" = "blanqueamientos.jpg",
  //   ],
  //   "diseño digital de la sonrisa" = [
  //     "text" = "Es un tratamiento personalizado a partir de la utilización de un diagnóstico preliminar fotográfico y un software, que permite diseñar su sonrisa y ver el resultado final antes de comenzar el tratamiento, para mejorar la anatomía estética y color de tu sonrisa. De esta manera, nos aseguraremos de que te estamos ofreciendo la solución que mejor se adapta a sus necesidades.",
  //     "pic" = "diseño digital de la sonrisa.jpg",
  //   ],
  //   "selladores de fosas y fisuras" = [
  //     "text" = "lalal",
  //     "pic" = "lolo",
  //   ],
  //   "tratamientos de conductos" = [
  //     "text" = "lalal",
  //     "pic" = "lolo",
  //   ],
  //   "brackets metálicos" = [
  //     "text" = "lalal",
  //     "pic" = "lolo",
  //   ],
  //   "brackets estéticos de porcelana" = [
  //     "text" = "lalal",
  //     "pic" = "lolo",
  //   ],
  //   "brackets estéticos de zafiro" = [
  //     "text" = "lalal",
  //     "pic" = "lolo",
  //   ],
  //   "alineadores invisibles SmileMakers®" = [
  //     "text" = "lalal",
  //     "pic" = "alineadores invisibles SmileMakers®.jpg",
  //   ],
  //   "alineadores keepsmiling" = [
  //     "text" = "lalal",
  //     "pic" = "alineadores keepsmiling.jpg",
  //   ],
  //   "alineadores invisiling" = [
  //     "text" = "lalal",
  //     "pic" = "alineadores invisiling.jpg",

  //   ],
  //   "prótesis removibles" = [
  //     "text" = "lalal",
  //     "pic" = "lolo",
  //   ],
  //   "prótesis fijas" = [
  //     "text" = "lalal",
  //     "pic" = "prótesis fijas.jpg",
  //   ],
  //   "carillas" = [
  //     "text" = "lalal",
  //     "pic" = "carillas.jpg",
  //   ],
  // ],

  specialties : [
    {
      name:"Odontología General",
      text : "Realizamos practicas mínimamente invasivas, poniendo énfasis en tu estética y en la prevensión de futuras lesiones, mediante restauraciones estéticas, limpieza y pulido dental.",
      treatments : [
        "caries",
        "limpieza ultrasónica",
        "exodoncias",
        "selladores de fosas y fisuras",
      ],
      phone : [5491167485220, 5491173606683],
      phone_text : ["Hola Dr. Nuñez, me gustaría hacerle una consulta sobre Odontologia General", "Hola Dra. Beneitone, me gustaría hacerle una consulta sobre Odontologia General"],
    },
    {
      name:"Implantes",
      text : "Ayudamos en la recuperación de tus piezas perdidas, con la ayuda de equipamiento de última tecnología. Practicamos cirugías guiadas, previo a una planificación digital que permite resultados asegurados.",
      treatments : [],
      phone : [5491173606683],
      phone_text : ["Hola Dr. Loccisano, me gustaría hacerle una consulta sobre Implantes"],
    },
    {
      name:"Endodoncia",
      text : "Utilizamos equipos tecnológicos de última generación, los tratamientos de conductos pueden dejar de ser un trauma gracias a la ayuda de nuestro especialista.",
      treatments : [
        "tratamientos de conductos",
        'retratamiento de conducto',
        'cirugía apical',
      ],
      phone : [5491167485220],
      phone_text : ["Hola Dr. Nuñez, me gustaría hacerle una consulta sobre Endodoncia"],
    },
    {
      name:"Ortodoncia",
      text : "Mediante un novedoso sistema de ortodoncia invisibles o técnicas convencionales de ortodoncia fija con diversos materiales, ayudamos en la alineación y estética de tus dientes.",
      treatments : [
        "alineadores invisibles SmileMakers®",
        "alineadores keepsmiling",
        "alineadores invisiling",
        "brackets metálicos",
        "brackets estéticos de porcelana",
        "brackets estéticos de zafiro",
      ],
      phone : [5491167485220],
      phone_text : ["Hola Dra. Loccisano, me gustaría hacerle una consulta sobre Ortodoncia"],
    },
    {
      name : "Prótesis",
      text : "Lo mas importate para nosotros no es unicamente tu sonrisa, sino devolverte la anatomía y funciones perdidas. Para ello realizamos rehabilitaciones fijas sobre implantes o dientes con coronas e incrustaciones estéticas de porcelana pura. Atrás quedaron las rehabilitaciones largas, el sistema CAD CAM (escaneo y fresado) nos permiten rehabilitaciones rápidas precisas y sumamente estéticas.",
      treatments : [
        'coronas de disilicato de litio',
        'incristaciones de disilicato de litio',
        'coronas de zafiro',
      ],
      phone : [5491173606683],
      phone_text : ["Hola Dr. Loccisano, me gustaría hacerle una consulta sobre Prótesis"],
    },
    {
      name: "Odontopediatría",
      text : "Las primeras consultas de la vida de tus hijos son las más importantes.Por ello, trabajamos con amor, dedicación y contencíon de los mas chicos en beneficio a su salud bucal.",
      treatments : [],
      phone : [5491173606683],
      phone_text : ["Hola Dra. Beneitone, me gustaría hacerle una consulta sobre Odontopediatría"],
    },
    {
      name: "Estética Dental",
      text : "Entendemos a tu sonrisa como tu carta de presentación, las primeras impresiones son unicas e irrepetibles. A partir de tratamientos individualizados vamos devolverte la confianza y esas ganas de sonreir que siempre deseaste.",
      treatments : [
        "blanqueamientos",
        "diseño digital de la sonrisa",
        "carillas",
      ],
      phone : [5491167485220, 5491173606683],
      phone_text : ["Hola Dr. Nuñez, me gustaría hacerle una consulta sobre Estética Dental", "Hola Dra. Beneitone, me gustaría hacerle una consulta sobre Estética Dental"],
    },
  ],

  team : [
    {
      code: 'mb',
      name : "maria victoria beneitone",
      title : "Odontopediatria y Rehabilitacion Estetica Adhesiva ",
      lic_num : "36.194",
      avatar : "od_mb.jpg",
    },
    {
      code: 'ml',
      name : "mercedes loccisano",
      title : "Especialista en Ortodoncia y Ortopedia de los Maxilares",
      lic_num : "36.700",
      avatar : "od_ml.jpg",
    },
    {
      code: 'gl',
      name : "guillermo loccisano",
      title : "Diplomatura en Implantología y Rehabilitación Oral",
      lic_num : "36.052",
      avatar : "od_gl.jpg",
    },
    {
      code: 'jn',
      name : "joaquin nuñez",
      title : "Especialista en Endodoncia y Rehabilitación Estética Adhesiva",
      lic_num : "36.785",
      avatar : "od_jn.jpg",
    },
  ]

}